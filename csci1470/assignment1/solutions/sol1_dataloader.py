...
from assignment import DataLoader
...
def main():
    ...
    print(dl.train_images)

    model = Model(dl.train_images, dl.train_labels)
    # for testing its just 5 but it should be 10k
    steps = int(1e4)
    for i in range(steps):
        model.run()
    print("Accuracy: {}".format(model.accuracy_function(dl.test_images, dl.test_labels)))
